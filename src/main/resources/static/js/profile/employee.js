function search() {
    ...
    $.ajax({
        ...
        timeout: 5000,
        dataType: 'json',
        url: "/employee/specialization/search",
        data: {
...
function save() {
    ...
    $("#specializations tr").each(function () {
        var id = $(this).attr('id')
        speclis.push(id.replace('specialization-', ''));
    });
    spinner.start();
    ...
    $.ajax({
        ...
        data: {
            ...
            prefix_title: $('input[name=prefix_title]').val(),
            suffix_title: $('input[name=suffix_title]').val(),
            speclist: speclis.join()
        },
        success: function (data) {
            loginCheck(data);
            spinner.stop();
